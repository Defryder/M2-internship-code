...
#ILP without environment constraints
#Same as LP counterpart, except the x decision variable is binary
def ILPS(Cmax, Tmax, M, N, K, c, p, d, b, env):
    
    m = Model("LP")
    
    x = [[m.add_var(var_type=BINARY) for j in range(N)] for i in range(M)]
...
#ILP with environment constraints
#Same as LP counterpart, except the x decision variable is binary
def ILP(Cmax, Tmax, M, N, K, c, p, d, b, env):
